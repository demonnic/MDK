...
--[[ Modifications for use in Mudlet by Damian Monogue <demonnic@gmail.com> ]]
...
M.FAILURE_PREFIX = 'LuaUnit test <red>FAILURE<reset>: ' -- prefix string for failed tests
M.SUCCESS_PREFIX = 'LuaUnit test <green>SUCCESS<reset>: ' -- prefix string for successful tests finished early
...
function M.wrapFunctions()
    ...
    -- so just do nothing !
    echo([[Use of WrapFunctions() is no longer needed.
...
TapOutput.__class__ = 'TapOutput'
    ...
    function TapOutput:startSuite()
        cecho("1.."..self.result.testCount .. "\n")
        cecho('# Started on '..self.result.startDate .. "\n")
    end
    function TapOutput:startClass(className)
        if className ~= '[TestFunctions]' then
            cecho('# Starting class: '..className .. "\n")
        end
    end

    function TapOutput:addStatus( node )
        cecho("not ok ", self.result.currentTestNumber, "\t", node.testName, "\n")
        if self.verbosity > M.VERBOSITY_LOW then
           cecho( prefixString( '#   ', node.msg ) .. "\n")
        end
        if self.verbosity > M.VERBOSITY_DEFAULT then
           cecho( prefixString( '#   ', node.stackTrace ) .. "\n" )
        end
    end

    function TapOutput:endTest( node )
        if node:isPassed() then
            cecho("ok     ", self.result.currentTestNumber, "\t", node.testName, "\n")
        end
    end

    function TapOutput:endSuite()
        cecho( '# '..M.LuaUnit.statusLine( self.result ) .. "\n")
...
JUnitOutput.__class__ = 'JUnitOutput'
    ...
    function JUnitOutput:startSuite()
        ...
        cecho('# XML output to '..self.fname .. "\n")
        cecho('# Started on '..self.result.startDate .. "\n")
    end
    ...
    function JUnitOutput:startTest(testName)
        cecho('# Starting test: '..testName .. "\n")
    end

    function JUnitOutput:addStatus( node )
        if node:isFailure() then
            cecho( '#   Failure: ' .. prefixString( '#   ', node.msg ):sub(4, nil) .. "\n" )
            -- print('# ' .. node.stackTrace)
        elseif node:isError() then
            cecho( '#   Error: ' .. prefixString( '#   '  , node.msg ):sub(4, nil) .. "\n")
            -- print('# ' .. node.stackTrace)
        end
    end
...
TextOutput.__class__ = 'TextOutput'
    ...
    function TextOutput:startTest(testName)
        if self.verbosity > M.VERBOSITY_DEFAULT then
            cecho( "    ", self.result.currentNode.testName, " ... \n" )
        end
    end
    ...
    function TextOutput:endTest( node )
        if node:isPassed() then
            if self.verbosity > M.VERBOSITY_DEFAULT then
                cecho("<green>Ok<reset>\n")
            else
                cecho("<green>.<reset>")
            end
        else
            if self.verbosity > M.VERBOSITY_DEFAULT then
                cecho( string.format("<red>%s<reset>\n", node.status ))
                cecho( string.format("<red>%s<reset>\n", node.msg ))
                --[[
                ...
                cecho(string.format("<red>%s<reset>" , string.sub(node.status, 1, 1)))
            end
        end
    end

    function TextOutput:displayOneFailedTest( index, fail )
        cecho(index..") <red>"..fail.testName .. "<reset>\n")
        cecho(fail.msg .. "<reset>\n" )
        cecho(fail.stackTrace .. "<reset>\n" )
        cecho("\n")
    end
...
if EXPORT_ASSERT_TO_GLOBALS then
    ...
    function M.LuaUnit.version()
        cecho('LuaUnit v'..M.VERSION..' by Philippe Fremy <phil@freehackers.org>' .. "\n")
        --os.exit(0)
    end
...
--
    ...
    local function conditional_plural(number, singular, color)
        ...
        return string.format('%s%d<reset> %s%s',color, number, singular, suffix)
    end
    ...
    function M.LuaUnit.statusLine(result)
        ...
        local s = {
            ...
            conditional_plural(result.passedCount, 'success', "<green>"),
        }
        if result.notPassedCount > 0 then
            if result.failureCount > 0 then
                table.insert(s, conditional_plural(result.failureCount, 'failure', "<red>"))
            end
            if result.errorCount > 0 then
                table.insert(s, conditional_plural(result.errorCount, 'error', "<orange>"))
            end
        else
            table.insert(s, '<green>0<reset> failures')
        end
    ...
    function M.LuaUnit:runSuiteByInstances( listOfNameAndInst )
        ...
        * { class.method name, class instance }
        ...
        if self.result.aborted then
            cecho("LuaUnit ABORTED (as requested by --error or --failure option)\n")
            --os.exit(-2)
        end
    end
    ...
    function M.LuaUnit:runSuite( ... )
        ...
        if options.output then
            if options.output:lower() == 'junit' and options.fname == nil then
                cecho("With junit output, a filename must be supplied with -n or --name\n")
                --os.exit(-1)
            end
